...
export default function Guests() {
  ...
  const iconLoading = clsx({
    [[styles.loader, styles.showLoginLoader].join(" ")]: postLoader || providerAuthLoader,
    [styles["loader"]]: !postLoader || !providerAuthLoader,
  });

  const textLoading = clsx({
    [[styles.loginTxtnIco, styles.hideLoginTxtnIco].join(" ")]: postLoader || providerAuthLoader,
  ...
  const textLoading1 = clsx({
    [[styles.loginTxtnIco, styles.hideLoginTxtnIco].join(" ")]: providerAuthLoader1,
        ...
        <div className={styles.interact}>
          {!authLoading ? (
            session != undefined ? (
              <div className={styles.create}>
                ...
                <form className="con" onSubmit={postEntry}>
                  <textarea ref={inputEl} placeholder="Your message" rows="4" required></textarea>
                  <button type="submit">
            ...
            <div class={styles.authwall}>
              ...
              <button className={styles.provider_btn} onClick={(e) => { providerAuth(1); signIn('github') }}>
              ...
              <button className={`${styles.provider_btn} ${styles.one}`} onClick={(e) => { providerAuth(2); signIn('google') }}>
                <span className={textLoading1}>
                  <img src="/icons/google.svg" alt="" className={styles.icon} />
          ...
          <div className={styles.messages}>
            {data && entries.length == 0 ? (<div className={styles.empty}>Empty 😥</div>) 
              : 
              (<Masonry
                        ...
                        <div className={styles.header}>
                          <div className={styles.con}>
                            ...
                            <div className={styles.info}>
                              <div className={styles.name}>{ dropMsg.name }</div>
                              <div className={styles.date}>{moment(dropMsg.updatedAt).calendar()}</div>
                            </div>
                          </div>
                        </div>
                        <div className={styles.message_body}>{ dropMsg.body }</div>
                      </div>
                    )
                  }))
                  :
